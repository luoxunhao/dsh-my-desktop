...
test('sandbox preload 的 IPC 字面量与主契约保持一致', async () => {
  ...
  assert.deepEqual(channelLiterals(shell), [SHELL_IPC.action, SHELL_IPC.bootstrap, SHELL_IPC.closeDesktopSettings, SHELL_IPC.desktopUpdateAction, SHELL_IPC.desktopUpdateState, SHELL_IPC.getBootstrap, SHELL_IPC.getDesktopUpdateState, SHELL_IPC.getNotificationPreferences, SHELL_IPC.getUpdatePreferences, SHELL_IPC.popupMenu, SHELL_IPC.popupTool, SHELL_IPC.settingsSection, SHELL_IPC.state, SHELL_IPC.tool, SHELL_IPC.updateNotificationPreferences, SHELL_IPC.updateUpdatePreferences, SHELL_IPC.windowControl, SHELL_IPC.windowState].sort())
