...
恢复社区插件预装，并把两处会让离线首启出问题的耦合修掉。

- **预装这三项**：`dshmarket`（可视化插件市场）、`@luoxunhao/dsh-codex-project` 0.13.0、
  `dsh-quote` 0.1.0 —— 出包时装配进离线仓库，首启零联网补种。**不再预装**
  `dsh-vision-router`（实测在 `0.1.7-alpha.1` 下渲染侧加载失败，离线与放网各验一次）与
  `dsh-context`。
- **新建 profile 默认开启插件市场**：`dshmarket` 随包但此前被"市场是 opt-in"这条挡住，首启装不上。
  现在没有状态文件（= 新 profile）即按开启处理；设置页开关照旧可关；已经写过选择的 profile
  完全保持原状。状态文件存在但读不动时仍按关闭处理 —— 一份信不过的文档不该拿产品默认值替它说话。
- **随包 store 自动带上运行时那侧的离线元数据**：pnpm 的元数据按 registry 域名分键，构建默认走
  镜像源、首启按默认源找 ⟹ 一次强制重装就能让离线补种全灭（`ERR_PNPM_NO_OFFLINE_META`）。
  现在装配完镜像源那遍后，会自动对官方源再解析一次元数据（`--lockfile-only`，不下 tarball）。
- **待更新应用那条路径也走随包元数据**：它不处于 offline 模式，此前因此没带 `--cache-dir`，
  离线机器上表现为"补种成功、插件更新静默失败"。现在 `--cache-dir` 跟着 store 走而不是跟着
  `--offline` 走。
