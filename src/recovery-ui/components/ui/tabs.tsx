/**
 * Tabs — the recovery page's panel switcher.
 *
 * Ported verbatim from the reference (`native-ui/components/ui/tabs.tsx`), built on
 * `@base-ui/react`'s Tabs primitive so keyboard navigation and ARIA wiring come from
 * the library rather than from hand-rolled state.
 *
 * `activateOnFocus` is kept from the reference: arrow keys move AND select, which is
 * the behaviour the recovery page was designed around.
 */
...
function Tabs({ className, ...props }: ComponentProps<typeof TabsPrimitive.Root>): React.JSX.Element {
...
function TabsTrigger({ className, ...props }: ComponentProps<typeof TabsPrimitive.Tab>): React.JSX.Element {
...
function TabsContent({ className, ...props }: ComponentProps<typeof TabsPrimitive.Panel>): React.JSX.Element {
