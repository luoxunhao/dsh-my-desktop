...
`settings.section`，提供 Profile 管理、插件市场选择、AA 开关、通知等设置区块。
...
- host 半边在 DSH server 进程运行：对可自洽的操作（插件市场选择、AA/通知偏好等）真实现持久化，
  对只属于 Electron 桌面壳的能力（重启、切换 profile、开终端、DevTools、诊断导出等）做**能力探测**，
...
```bash
pnpm install              # 根目录；官方 client 类型来自 plugins/dsh-my-desktop-settings/vendor/0.1.6-alpha.2 tgz（与 dsh 0.1.6-alpha.2 对齐）
